...
export default function AdminV2Team() {
  ...
  const loadOverloaded = useCallback(async () => {
    try {
      const r = await axios.get(`${API}/admin/team/overloaded`, { withCredentials: true });
      setOverloaded(r.data?.items || []);
    } catch {
      setOverloaded([]);
    } finally {
      setLoading(false);
  ...
  const rebalanceDev = async (devId, devName) => {
    ...
    setBusy(devId);
    try {
      const r = await axios.post(`${API}/admin/team/rebalance/${devId}`, {}, { withCredentials: true });
      const movesCount = r.data?.moves_count || 0;
      alert(movesCount > 0
  ...
  const rebalanceAll = async () => {
    ...
    setRunningAll(true);
    try {
      const r = await axios.post(`${API}/admin/team/auto-rebalance`, {}, { withCredentials: true });
      alert(`✅ Done. Total moves: ${r.data?.total_moves || 0}`);
      await loadOverloaded();
    } catch (e) {
      alert(`Failed: ${e?.response?.data?.detail || 'error'}`);
    } finally {
