...
function MultiCheckbox({ value, onChange, options, testid }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-2" data-testid={testid}>
      {options.map(([v, l]) => (
        <label key={v} className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-xs ${set.has(v) ? 'border-blue-500/60 bg-blue-900/30 text-blue-100' : 'border-border bg-muted text-muted-foreground hover:bg-muted/60'}`}>
          <input type="checkbox" checked={set.has(v)} onChange={() => toggle(v)} className="hidden" />
          <span className={`w-3 h-3 rounded border-2 ${set.has(v) ? 'bg-blue-500 border-blue-500' : 'border-muted-foreground'}`} />
          <span>{l}</span>
        </label>
      ))}
    </div>
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer" data-testid={testid}>
      <button
        ...
        className={`relative w-10 h-6 rounded-full transition ${value ? 'bg-blue-500' : 'bg-muted border border-border'}`}
      >
...
function SaveBtn({ onClick, busy, label = 'Save', testid }) {
  return (
    <button
      ...
      disabled={busy}
      data-testid={testid}
      className="inline-flex items-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-40 px-4 py-2 text-sm font-medium text-white"
    >
...
export default function AdminPaymentsPage() {
  ...
  if (loading) return <div className="p-12 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-blue-400" /></div>;
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[
            ...
            <button
              ...
              className={`rounded-lg border px-3 py-3 text-left ${appForm.active_payment_provider === v ? 'border-blue-500 bg-blue-900/30' : 'border-border bg-muted hover:bg-muted/60'}`}
            >
